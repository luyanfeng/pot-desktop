...
async function niutrans_detect(text) {
    ...
    let res = await fetch(
        `https://test.niutrans.com/NiuTransServer/language?${new URLSearchParams({
            src_text: text,
            source: 'text',
...
async function yandex_detect(text) {
    ...
    let res = await fetch(
        `https://translate.yandex.net/api/v1/tr.json/detect?${new URLSearchParams({
...
async function bing_detect(text) {
    ...
    if (token.ok) {
        ...
        let res = await fetch(
            `https://api-edge.cognitive.microsofttranslator.com/detect?${new URLSearchParams({
                'api-version': '3.0',
            })}`,
            {
                method: 'POST',
